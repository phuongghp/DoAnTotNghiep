...
# Định nghĩa các môn có tên khác nhau giữa học phần bảng điểm và CTĐT
# Key: Tên học phần ở bảng điểm
# Value: Tên học phần tương ứng ở CTĐT
...
#Định nghĩa danh sách các học kỳ loại bỏ
hoc_ky_loai_bo = []
...
def read_diem_tot_nghiep(file_path):
    ...
    for sheet_name in sheet_names:
        ...
        if diem_ca_nhan['xep_loai_hoc_tap'][0] == "X":
            diem_ca_nhan['xep_loai_hoc_tap'] = "Xuat sac"
        elif diem_ca_nhan['xep_loai_hoc_tap'][0] == "T":
            diem_ca_nhan['xep_loai_hoc_tap'] = "Trung binh"
        elif diem_ca_nhan['xep_loai_hoc_tap'][0] == "G":
            diem_ca_nhan['xep_loai_hoc_tap'] = "Gioi"
...
# %%  
# Trích xuất điểm sinh viên chưa tốt nghiệp
...
# %%
# Chuyển bảng điểm đầy đủ sang môn tự chọn
    ...
    for i in range(len(bang_diem_tong_hop)):
        ...
        for key in dict_tu_chon.keys():
            ...
            for hoc_phan in dict_tu_chon[key].keys(): 
                ...
                hk_hp[f'montuchon{key}'] = hk_hp[hoc_phan]
                tc_hp[f'montuchon{key}'] = tc_hp[hoc_phan]
...
file_excel = "../BangDiem/DiemTrungBinh.xlsx"
